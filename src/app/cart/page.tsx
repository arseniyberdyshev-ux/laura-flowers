          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <section className="lg:col-span-7 bg-white/60 backdrop-blur-md p-8 rounded-3xl shadow-sm border border-white/50 h-fit">
              ...
              <div className="space-y-4">
                {cartProducts.map(p => (
                  <div key={p.id} className="flex justify-between items-center bg-white/40 p-4 rounded-2xl border border-white/40 hover:shadow-sm transition-all">
                    <div className="flex gap-5 items-center">
                       ...
                       <div className="flex flex-col">
                        ...
                        <button 
                          ...
                          className="text-[10px] text-rose-400 uppercase tracking-widest hover:text-rose-600 transition-colors self-start font-bold bg-rose-50 px-3 py-1.5 rounded-lg"
                        >
                          Удалить
                        </button>
                       </div>
                    </div>
                    <p className="text-lg font-light text-gray-800 whitespace-nowrap">{(p.price * p.quantity).toLocaleString('ru-RU')} ₽</p>
                  </div>
                ))}
              </div>
              <div className="text-xl font-light pt-8 mt-6 flex justify-between items-center tracking-widest border-t border-white/50">
                <span className="text-gray-500 text-sm font-medium">ИТОГО:</span>
                <span className="text-3xl text-gray-900">{total.toLocaleString('ru-RU')} ₽</span>
              </div>
            </section>

            {/* Правая часть: Форма доставки */}
